...
//What is the greatest product of four adjacent numbers in the same direction in the 20 x 20 grid?


fn scan_grid(grid: &Vec<Vec<u64>>) -> u64 {
    let mut max: u64 = 0;
    ...
    for i in top..bottom {
        for j in left..right {
            ...
            if right - j >= 4 {
                ...
                if prod > max { 
                    println!("New high score {} horizontally from ({}, {}).", prod, i, j); 
                    max = prod; }
            }
            /* Check vertical */
            if bottom - i >= 4 {
                ...
                if prod > max { 
                    println!("New high score {} vertically from ({}, {}).", prod, i, j); 
                    max = prod; }
            }

            /* Check diagonal left to right*/
            if right - j >= 4 && bottom - i >= 4 {
                let mut prod = 1;
                for k in 0..4 {
                    prod *= grid[i + k][j + k];
                }
                if prod > max { 
                    println!("New high score {} diagonally from ({}, {}).", prod, i, j); 
                    max = prod; }
            }
